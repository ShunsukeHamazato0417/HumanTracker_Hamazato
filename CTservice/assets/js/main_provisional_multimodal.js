...
worker.addEventListener('message', function(msg){
  ...
  integration_5(msg.data[0],msg.data[1]);
}, false);
...
function integration_5(json_c,json_d){
  //対応表の更新を行う
  for(let i=0;i<correspondence_id.length;i++){
    if(json_c.existing_id.indexOf(correspondence_id[i][0])!=-1){
      ...
      var camera_x = json_c.data[json_c.existing_id.indexOf(camera_id)].slice(-1)[0][0];
      var camera_y = json_c.data[json_c.existing_id.indexOf(camera_id)].slice(-1)[0][1];

      for(let j = 0;j<json_d.objects.length;j++){
        if(lidar_id==json_d.objects[j].id){
          var lidar_x =json_d.objects[j].cluster.centroid.x;
          var lidar_y =json_d.objects[j].cluster.centroid.y;
          var data_distance = Math.sqrt( Math.pow(camera_x - lidar_x, 2 ) + Math.pow( camera_y - lidar_y, 2 ));
          if(data_distance>4000){
            d_correspond_id.splice(d_correspond_id.indexOf(lidar_id),1);
            for(let k=0;k<json_d.objects.length;k++){
              var new_lidar_point = json_d.objects[k].cluster.centroid;
              var new_data_distance = Math.sqrt( Math.pow(camera_x - new_lidar_point.x, 2 ) + Math.pow( camera_y - new_lidar_point.y, 2 ));
              var min = 9999;
            ...
            d_correspond_id.push(correspondence_id[i][1]);
          }
          break;
        }
      }
    }else{ //ここで対応表の削除
      if(d_correspond_id.indexOf(correspondence_id[i][1])!=-1){
        d_correspond_id.splice(d_correspond_id.indexOf(correspondence_id[i][1]),1);
      }
  ...
  for(let i=0;i<json_c.existing_id.length;i++){
    if(c_correspond_id.indexOf(json_c.existing_id[i])==-1){
      ...
      var camera_x = json_c.data[i].slice(-1)[0][0];
      var camera_y = json_c.data[i].slice(-1)[0][1];
      var min = 9999;
      for(let j=0;j<json_d.objects.length;j++){
        var lidar_point = json_d.objects[j].cluster.centroid;
        var data_distance = Math.sqrt( Math.pow(camera_x - lidar_point.x, 2 ) + Math.pow( camera_y - lidar_point.y, 2 ));
        if(data_distance < 2000){
          ...
          if(closeness<min){
            if(min == 9999){
              ...
              console.log("push detected_id");
              console.log(correspondence_id[c_correspond_id.indexOf(json_c.existing_id[i])]);
            }else{
      ...
      d_correspond_id.push(correspondence_id[c_correspond_id.indexOf(json_c.existing_id[i])][1]);
    }
  }
  console.log(d_correspond_id);
  

// new_multimodal_dataの更新、削除
  for(let i=0;i<new_multimodal_data.length;i++){ //new_multimodal_dataのcamera_idとlidar＿idを更新
    new_multimodal_data[i].time = json_d.time;
    var camera_id = new_multimodal_data[i].camera_id; 
    var lidar_id = new_multimodal_data[i].lidar_id;
    if(camera_id){
      new_multimodal_data[i].type = 'human';
    }else{
      for(let j=0;j<json_d.objects.length;j++){
        if(json_d.objects[j].id==lidar_id){
          new_multimodal_data[i].type = json_d.objects[j].type.current;
        }
      }
      
    }
    if(c_correspond_id.indexOf(camera_id)==-1){
      if(d_correspond_id.indexOf(lidar_id)!=-1){
        for(let j=0;j<correspondence_id.length;j++){
          if(correspondence_id[j][1]==lidar_id){
            new_multimodal_data[i].camera_id=correspondence_id[j][0];
          }
        }
      }
    }
    if(d_correspond_id.indexOf(lidar_id)==-1){
      if(c_correspond_id.indexOf(camera_id)!=-1){
        for(let j=0;j<correspondence_id.length;j++){
          if(correspondence_id[j][0]==camera_id){
            new_multimodal_data[i].lidar_id=correspondence_id[j][1];
          }
        }
      }
    }
  }
  for(let i=0;i<new_multimodal_data.length;i++){//座標の更新
    var camera_id = new_multimodal_data[i].camera_id;
    var lidar_id =new_multimodal_data[i].lidar_id;
    if(lidar_id == null){
      for(let j=0;j<json_c.existing_id.length;j++){
        if(camera_id==json_c.existing_id[j]){
          new_multimodal_data[i].history.push([new_multimodal_data[i].x,new_multimodal_data[i].y]);
          if(new_multimodal_data[i].history.length>6){
            new_multimodal_data[i].history.shift();
          }
          new_multimodal_data[i].x = json_c.data[j].slice(-1)[0][0];
          new_multimodal_data[i].y = json_c.data[j].slice(-1)[0][1];
        }
      }
    }else{
      for(let j=0;j<json_d.objects.length;j++){
        if(lidar_id==json_d.objects[j].id){
          ...
          new_multimodal_data[i].x = json_d.objects[j].cluster.centroid.x;
          new_multimodal_data[i].y = json_d.objects[j].cluster.centroid.y;
        }
      }
    }
  }
  //new_multimodal_dataの削除
  var camera_exisiting_id = [];
  var lidar_exisiting_id = [];
  for(let i=0;i<json_c.existing_id.length;i++){
    camera_exisiting_id.push(json_c.existing_id[i]);
  }
  for(let i=0;i<json_d.objects.length;i++){
    lidar_exisiting_id.push(json_d.objects[i].id);
  }
  for(let i=0;i<new_multimodal_data.length;i++){
    var camera_id = new_multimodal_data[i].camera_id;
    var lidar_id =new_multimodal_data[i].lidar_id;
    // console.log(new_multimodal_data[i].lidar_id);
    // console.log(lidar_exisiting_id);
    // console.log(lidar_exisiting_id.indexOf(lidar_id));
    if(camera_exisiting_id.indexOf(camera_id)==-1 && lidar_exisiting_id.indexOf(lidar_id)==-1){
      new_multimodal_data.splice(i,1);
      i--;
    }
  }
  ...
  // new_multimodal_dataの生成
  var new_multimodal_data_exisiting_camera_id = [];
  var new_multimodal_data_exisiting_lidar_id = [];
  for(let i=0;i<new_multimodal_data.length;i++){
    new_multimodal_data_exisiting_camera_id.push(new_multimodal_data[i].camera_id);
  }
  for(let i=0;i<new_multimodal_data.length;i++){
    new_multimodal_data_exisiting_lidar_id.push(new_multimodal_data[i].lidar_id);
  }
  // カメラでの検出範囲内での生成
  for(let i=0;i<correspondence_id.length;i++){
    var camera_id = correspondence_id[i][0];
    var lidar_id = correspondence_id[i][1];
    if(new_multimodal_data_exisiting_camera_id.indexOf(camera_id)==-1){
      for(let j=0;j<json_d.objects.length;j++){
        if(json_d.objects[j].id==lidar_id){
          new_multimodal_data.push({
            "id" : camera_id,
            "camera_id" : camera_id,
            "lidar_id" : correspondence_id[i][1],
            "time" : json_d.time,
            "status" : "lidar",
            "type" : 'human',
            "x" : json_d.objects[j].cluster.centroid.x,
            "y" : json_d.objects[j].cluster.centroid.y, 
            "history" : []
          })
        }
      }
  ...
  // 測域センサの範囲での生成
  for(let i=0; i<json_d.objects.length; i++){
    var lidar_id = json_d.objects[i].id;
    if(new_multimodal_data_exisiting_lidar_id.indexOf(lidar_id)==-1){
      new_multimodal_data.push({
        "id" : lidar_id,
        "camera_id" : null,
        "lidar_id" : lidar_id,
        "time" : json_d.time,
        "status" : "lidar",
        "type" : json_d.objects[i].type.current,
        "x" : json_d.objects[i].cluster.centroid.x,
        "y" : json_d.objects[i].cluster.centroid.y, 
        "history" : []
      })
    }
  }
  for(let i=0;i<new_multimodal_data.length;i++){
    console.log(new_multimodal_data[i].type);
  }
  display_multimodal_data(new_multimodal_data);
}
