...
########################*/
// Worker定義 
const worker = new Worker(new URL('./worker_history_multimodal_sub.js', import.meta.url));
worker.postMessage({
	ty: ty
})
...
worker.addEventListener('message', function(msg){
  ...
  display_sensor_data(msg.data[0])
  // if(msg.data[0].data[0]){one_display_sensor_data(msg.data[0].data[0]);}
//   integration_3(msg.data[0],msg.data[1]);
}, false);
...
function one_display_sensor_data(json,i){
  ...
  if(json){
    for (var j=0; j<json.length; j ++){
      if(i){
        ...
        var c = colors[json[j][2] % 9];
      }
      

      data.beginFill(c);
      data.drawRect(json[j][0] - data_size/2, 
                    json[j][1] - data_size/2,
          data_size*2, data_size*2)
      data.endFill();
    }
...
function display_multimodal_data(json){
  // time_display.text = "Time: "+ json[0].time;
  displaying = true;
  scaling_ratio = app_w / display_w;
  expantion_ratio = display_ratio * app_display_ratio * scaling_ratio;
  data.setTransform(-display_min_x*scaling_ratio, -display_min_y*scaling_ratio, expantion_ratio, expantion_ratio);
  ...
  for (var i=0; i<json.length; i++){
    if(json[i].id){
      time_display.text = "Time: "+ json[i].time.year +"/" + json[i].time.mon + "/"+json[i].time.day+"  "+json[i].time.hour+":"+json[i].time.min+":"+json[i].time.sec+"."+json[i].time.misec;
      var c = colors[json[i].id % 9];
        ...
        data.drawRect(json[i].x - data_size/2, 
            ...
            data_size*2, data_size*2)
        data.endFill();
      for (var j=0; j<json[i].history.length; j++){
        var c = colors[json[i].id % 9];

        data.beginFill(c);
        data.drawRect(json[i].history[j][0] - data_size/2, 
                      json[i].history[j][1] - data_size/2,
            data_size*2, data_size*2)
        data.endFill();
      }
    }
...
var correspondence_id = [];
var c_correspond_id = [];
var d_correspond_id = [];
var c_identification = [];
var c_identification_sub = [];
var complementary_data = [];
var multimodal_data = [];

function integration(json_c,json_d){
  complementary_data =[];
  if(json_c.time_history.length==3){
    // console.log(json_c.time_history[0]);
    var camera_flame_time = to_date_c(json_c.time_history[2]).getTime() - to_date_c(json_c.time_history[0]).getTime();
    
    var lidar_flame = Math.trunc((camera_flame_time/1000)*20);
    if(lidar_flame>19){lidar_flame=19;}
    // console.log(lidar_flame);
    for(let i=0;i<json_c.new_existing_id.length;i++){
      if(c_correspond_id.indexOf(json_c.new_existing_id[i])==-1 && json_c.existing_id.indexOf(json_c.new_existing_id[i])!=-1){
        // console.log(json_c.new_existing_id);
        correspondence_id.push([json_c.new_existing_id[i]]);
        c_correspond_id.push(json_c.new_existing_id[i]);
        var c_x = json_c.data[json_c.existing_id.indexOf(json_c.new_existing_id[i])].slice(-1)[0][0];
        // console.log(c_x);
        var c_move_x = c_x - json_c.data[json_c.existing_id.indexOf(json_c.new_existing_id[i])][0][0];
        var c_y = json_c.data[json_c.existing_id.indexOf(json_c.new_existing_id[i])].slice(-1)[0][1];
        var c_move_y = c_y - json_c.data[json_c.existing_id.indexOf(json_c.new_existing_id[i])][0][1];
        var min = 9999;
        
        for(let j=0;j<json_d.objects.length;j++){
          var lidar_point = json_d.objects[j].cluster.centroid;
          var data_distance = Math.sqrt( Math.pow(c_x - lidar_point.x, 2 ) + Math.pow( c_y - lidar_point.y, 2 ));
          if(data_distance < 2000){
            if(json_d.objects[j].history.log.length==20 && json_c.data[json_c.existing_id.indexOf(json_c.new_existing_id[i])].length==3){
              // correspondence_id[c_correspond_id.indexOf(json_c.new_existing_id[i])].push(json_d.objects[j].id);
              var x_closeness = (json_d.objects[j].history.log[json_d.objects[j].history.log.length-1].location.x - json_c.data[json_c.existing_id.indexOf(json_c.new_existing_id[i])][2][0]
                              + json_d.objects[j].history.log[json_d.objects[j].history.log.length-1-Math.trunc(lidar_flame/2)].location.x - json_c.data[json_c.existing_id.indexOf(json_c.new_existing_id[i])][1][0]
                              + json_d.objects[j].history.log[json_d.objects[j].history.log.length-1-lidar_flame].location.x - json_c.data[json_c.existing_id.indexOf(json_c.new_existing_id[i])][0][0])/3;
              var y_closeness = (json_d.objects[j].history.log[json_d.objects[j].history.log.length-1].location.y - json_c.data[json_c.existing_id.indexOf(json_c.new_existing_id[i])][2][1]
                              + json_d.objects[j].history.log[json_d.objects[j].history.log.length-1-Math.trunc(lidar_flame/2)].location.y - json_c.data[json_c.existing_id.indexOf(json_c.new_existing_id[i])][1][1]
                              + json_d.objects[j].history.log[json_d.objects[j].history.log.length-1-lidar_flame].location.y - json_c.data[json_c.existing_id.indexOf(json_c.new_existing_id[i])][0][1])/3;
              var closeness = Math.trunc(Math.sqrt( Math.pow(x_closeness, 2 ) + Math.pow( y_closeness, 2 )));
            }else{
              var x_closeness = json_d.objects[j].history.log[json_d.objects[j].history.log.length-1].location.x - json_c.data[json_c.existing_id.indexOf(json_c.new_existing_id[i])].slice(-1)[0][0];
              var y_closeness = json_d.objects[j].history.log[json_d.objects[j].history.log.length-1].location.y - json_c.data[json_c.existing_id.indexOf(json_c.new_existing_id[i])].slice(-1)[0][1];
              var closeness = Math.trunc(Math.sqrt( Math.pow(x_closeness, 2 ) + Math.pow( y_closeness, 2 )));
            }
            if(closeness<min){
              if(min == 9999){
                ...
                correspondence_id[c_correspond_id.indexOf(json_c.new_existing_id[i])][1] = json_d.objects[j].id;
              }
              min = closeness;
            }
          } 
    ...
    // for(let i=0;i<json_c.delete_id.length;i++){
    //   if(c_correspond_id.indexOf(json_c.delete_id[i])>-1){
    //     correspondence_id.splice(c_correspond_id.indexOf(json_c.delete_id[i]),1);
    //     c_correspond_id.splice(c_correspond_id.indexOf(json_c.delete_id[i]),1);
    //   }
    // }
    for(let i=0; i<c_correspond_id.length; i++){
      var c_id = c_correspond_id[i];
      if(json_c.existing_id.indexOf(c_id)==-1){
        correspondence_id.splice(i,1);
        c_correspond_id.splice(i,1);
      }
    }
    console.log(correspondence_id);
  }
  for(let i=0;i<correspondence_id.length;i++){   //対応させた組が２m離れたら再アサイン
    if(json_c.existing_id.indexOf(correspondence_id[i][0])!=-1){
      var c_x = json_c.data[json_c.existing_id.indexOf(correspondence_id[i][0])].slice(-1)[0][0];
      var c_y = json_c.data[json_c.existing_id.indexOf(correspondence_id[i][0])].slice(-1)[0][1];
      for(let j=0;j<json_d.objects.length;j++){
        if(json_d.objects[j].id==correspondence_id[i][1]){
          var lidar_point = json_d.objects[j].cluster.centroid;
          var data_distance = Math.sqrt( Math.pow(c_x - lidar_point.x, 2 ) + Math.pow( c_y - lidar_point.y, 2 ));
          if(data_distance>4000){
            for(let k=0;k<json_d.objects.length;k++){
              var new_lidar_point = json_d.objects[k].cluster.centroid;
              var new_data_distance = Math.sqrt( Math.pow(c_x - new_lidar_point.x, 2 ) + Math.pow( c_y - new_lidar_point.y, 2 ));
              var min = 9999;
              if(new_data_distance<2000){
                min = new_data_distance;
                correspondence_id[i][1] = json_d.objects[k].id;
              }
            }
          }
          break;
  ...
  // if(c_correspond_id){
  //   console.log(c_correspond_id);
  //   console.log(correspondence_id);
  // }
  var correspondence_id_flag = [];
  var len = correspondence_id.length;
  for (var i = 0; i < len; i++){
    correspondence_id_flag[i] = 0;  // 0 で初期化
  }
  for(let i = 0; i<correspondence_id.length; i++){
    var lidar_id = correspondence_id[i][1];
    var camera_id = correspondence_id[i][0];
    // console.log(lidar_id);
    for(let j = 0; j<json_d.objects.length; j++){
      if(lidar_id == json_d.objects[j].id){
        correspondence_id_flag[i]=1;
      }
    }
  }
  // console.log(correspondence_id_flag);
  for(let i=0; i < correspondence_id_flag.length; i++){
    if(correspondence_id_flag[i]==0){
      console.log(correspondence_id[i]);
      for(let j = 0; j<json_c.existing_id.length; j++){
        if(correspondence_id[i][0]==json_c.existing_id[j]){
          // complementary_data.push([json_c.data[j],correspondence_id[i][1]]);
          one_display_sensor_data(json_c.data[j],correspondence_id[i][1])
          console.log("one_display_sensor_data");
          break;
        }
      }
    
      var camera_id = correspondence_id[i][0];
      if(json_c.existing_id.indexOf(camera_id)!=-1){
        var c_x = json_c.data[json_c.existing_id.indexOf(camera_id)].slice(-1)[0][0];
        var c_y = json_c.data[json_c.existing_id.indexOf(camera_id)].slice(-1)[0][1];
        var min = 9999;
        for(let j=0;j<json_d.objects.length;j++){
          var lidar_point = json_d.objects[j].cluster.centroid;
          var data_distance = Math.sqrt( Math.pow(c_x - lidar_point.x, 2 ) + Math.pow( c_y - lidar_point.y, 2 ));
          if(data_distance < 2000){
            if(json_d.objects[j].history.log.length==20){
              var x_closeness = (json_d.objects[j].history.log[json_d.objects[j].history.log.length-1].location.x - json_c.data[json_c.existing_id.indexOf(camera_id)][2][0]
                              + json_d.objects[j].history.log[json_d.objects[j].history.log.length-1-Math.trunc(lidar_flame/2)].location.x - json_c.data[json_c.existing_id.indexOf(camera_id)][1][0]
                              + json_d.objects[j].history.log[json_d.objects[j].history.log.length-1-lidar_flame].location.x - json_c.data[json_c.existing_id.indexOf(camera_id)][0][0])/3;
              var y_closeness = (json_d.objects[j].history.log[json_d.objects[j].history.log.length-1].location.y - json_c.data[json_c.existing_id.indexOf(camera_id)][2][1]
                              + json_d.objects[j].history.log[json_d.objects[j].history.log.length-1-Math.trunc(lidar_flame/2)].location.y - json_c.data[json_c.existing_id.indexOf(camera_id)][1][1]
                              + json_d.objects[j].history.log[json_d.objects[j].history.log.length-1-lidar_flame].location.y - json_c.data[json_c.existing_id.indexOf(camera_id)][0][1])/3;
              var closeness = Math.trunc(Math.sqrt( Math.pow(x_closeness, 2 ) + Math.pow( y_closeness, 2 )));
            }else{
              var x_closeness = json_d.objects[j].history.log[json_d.objects[j].history.log.length-1].location.x - json_c.data[json_c.existing_id.indexOf(camera_id)][2][0];
              var y_closeness = json_d.objects[j].history.log[json_d.objects[j].history.log.length-1].location.y - json_c.data[json_c.existing_id.indexOf(camera_id)][2][1];
            ...
            if(closeness<min){
              correspondence_id[c_correspond_id.indexOf(camera_id)][1] = json_d.objects[j].id;
              min = closeness;
            }
          } 
        }
    ...
    var camera_id = correspondence_id[i][0];
    var lidar_id = correspondence_id[c_correspond_id.indexOf(camera_id)][1];
    for(let j=0;j<correspondence_id.length;j++){
      if(correspondence_id[j][1]==lidar_id){
        if(camera_id!=correspondence_id[j][0]){
          c_identification.push([camera_id,correspondence_id[j][0]])
        }
      }
    }
  }
  
  for(let i=0;i<c_identification.length;i++){
    var handle_lidar_id_a;
    var handle_lidar_id_b;
    var camera_id_a = c_identification[i][0];
    if(json_c.existing_id.indexOf(camera_id_a)!=-1){
      var c_x = json_c.data[json_c.existing_id.indexOf(camera_id_a)].slice(-1)[0][0];
      var c_y = json_c.data[json_c.existing_id.indexOf(camera_id_a)].slice(-1)[0][1];
      for(let j=0;j<json_d.objects.length;j++){
        var lidar_point = json_d.objects[j].cluster.centroid;
        var data_distance = Math.sqrt( Math.pow(c_x - lidar_point.x, 2 ) + Math.pow( c_y - lidar_point.y, 2 ));
        if(data_distance < 2000){
          if(json_d.objects[j].history.log.length-1>lidar_flame && json_c.data[json_c.existing_id.indexOf(camera_id_a)].length==3){
            // correspondence_id[c_correspond_id.indexOf(json_c.new_existing_id[i])].push(json_d.objects[j].id);
            var x_closeness = (json_d.objects[j].history.log[json_d.objects[j].history.log.length-1].location.x - json_c.data[json_c.existing_id.indexOf(camera_id_a)][0][0]
                            + json_d.objects[j].history.log[json_d.objects[j].history.log.length-1-lidar_flame].location.x - json_c.data[json_c.existing_id.indexOf(camera_id_a)][2][0])/2;
            var y_closeness = (json_d.objects[j].history.log[json_d.objects[j].history.log.length-1].location.y - json_c.data[json_c.existing_id.indexOf(camera_id_a)][0][1]
                            + json_d.objects[j].history.log[json_d.objects[j].history.log.length-1-lidar_flame].location.y - json_c.data[json_c.existing_id.indexOf(camera_id_a)][2][1])/2;
            var closeness = Math.trunc(Math.sqrt( Math.pow(x_closeness, 2 ) + Math.pow( y_closeness, 2 )));
            if(closeness<min){
              handle_lidar_id_a=json_d.objects[j].id;
              min = closeness;
            }
          }else{
            var x_closeness = json_d.objects[j].history.log[json_d.objects[j].history.log.length-1].location.x - json_c.data[json_c.existing_id.indexOf(camera_id_a)].slice(-1)[0][0];

            var y_closeness = json_d.objects[j].history.log[json_d.objects[j].history.log.length-1].location.y - json_c.data[json_c.existing_id.indexOf(camera_id_a)].slice(-1)[0][1];
    ...
    var camera_id_b = c_identification[i][1];
    if(json_c.existing_id.indexOf(camera_id_b)!=-1){
      var c_x = json_c.data[json_c.existing_id.indexOf(camera_id_b)].slice(-1)[0][0];
      var c_y = json_c.data[json_c.existing_id.indexOf(camera_id_b)].slice(-1)[0][1];
      for(let j=0;j<json_d.objects.length;j++){
        var lidar_point = json_d.objects[j].cluster.centroid;
        var data_distance = Math.sqrt( Math.pow(c_x - lidar_point.x, 2 ) + Math.pow( c_y - lidar_point.y, 2 ));
        if(data_distance < 2000){
          // correspondence_id[c_correspond_id.indexOf(json_c.new_existing_id[i])].push(json_d.objects[j].id);
          if(json_d.objects[j].history.log.length-1>lidar_flame && json_c.data[json_c.existing_id.indexOf(camera_id_b)].length==3){
            var x_closeness = (json_d.objects[j].history.log[json_d.objects[j].history.log.length-1].location.x - json_c.data[json_c.existing_id.indexOf(camera_id_b)][2][0]
                            + json_d.objects[j].history.log[json_d.objects[j].history.log.length-1-lidar_flame].location.x - json_c.data[json_c.existing_id.indexOf(camera_id_b)][0][0])/2;
            var y_closeness = (json_d.objects[j].history.log[json_d.objects[j].history.log.length-1].location.y - json_c.data[json_c.existing_id.indexOf(camera_id_b)][2][1]
                            + json_d.objects[j].history.log[json_d.objects[j].history.log.length-1-lidar_flame].location.y - json_c.data[json_c.existing_id.indexOf(camera_id_b)][0][1])/2;
            var closeness = Math.trunc(Math.sqrt( Math.pow(x_closeness, 2 ) + Math.pow( y_closeness, 2 )));
            if(closeness<min){
              handle_lidar_id_b=json_d.objects[j].id;
              min = closeness;
            }
          }else{
            var x_closeness = json_d.objects[j].history.log[json_d.objects[j].history.log.length-1].location.x - json_c.data[json_c.existing_id.indexOf(camera_id_b)].slice(-1)[0][0];
            var y_closeness = json_d.objects[j].history.log[json_d.objects[j].history.log.length-1].location.y - json_c.data[json_c.existing_id.indexOf(camera_id_b)].slice(-1)[0][1];
            var closeness = Math.trunc(Math.sqrt( Math.pow(x_closeness, 2 ) + Math.pow( y_closeness, 2 )));
            if(closeness<min){
              handle_lidar_id_b=json_d.objects[j].id;
              min = closeness;
      ...
      if(handle_lidar_id_a!=handle_lidar_id_b && handle_lidar_id_b){
        if(c_correspond_id.indexOf(camera_id_a)!=-1){
          correspondence_id[c_correspond_id.indexOf(camera_id_a)][1] = handle_lidar_id_a;
        }
        if(c_correspond_id.indexOf(camera_id_b)!=-1){
          correspondence_id[c_correspond_id.indexOf(camera_id_b)][1] = handle_lidar_id_b;
        }
        
        c_identification.splice(i,1);
        console.log("Reidentification");
        
      }else{
        console.log("not_Reidentification");
        one_display_sensor_data(json_c.data[json_c.existing_id.indexOf(camera_id_a)],handle_lidar_id_a);
        one_display_sensor_data(json_c.data[json_c.existing_id.indexOf(camera_id_b)],handle_lidar_id_b);
      }
    }
  }
  // one_display_sensor_data(complementary_data);
}

function integration_2(json_c,json_d){
  complementary_data =[];
  if(json_c.time_history.length==3){
    // console.log(json_c.time_history[0]);
    var camera_flame_time = to_date_c(json_c.time_history[2]).getTime() - to_date_c(json_c.time_history[0]).getTime();
    ...
    for(let i=0;i<json_c.existing_id.length;i++){
      if(c_correspond_id.indexOf(json_c.existing_id[i])==-1){
        // console.log(json_c.new_existing_id);
        correspondence_id.push([json_c.existing_id[i]]);
        multimodal_data.push({"id" : json_c.existing_id[i], "time" : json_c.det_time, "x" : json_c.data[i].slice(-1)[0][0], "y" : json_c.data[i].slice(-1)[0][1], "history" : []});
        c_correspond_id.push(json_c.existing_id[i]);
        var c_x = json_c.data[i].slice(-1)[0][0];
        // console.log(c_x);
        // var c_move_x = c_x - json_c.data[i][0][0];
        var c_y = json_c.data[i].slice(-1)[0][1];
        // var c_move_y = c_y - json_c.data[i][0][1];
        var min = 9999;
        ...
        for(let j=0;j<json_d.objects.length;j++){
          ...
          var data_distance = Math.sqrt( Math.pow(c_x - lidar_point.x, 2 ) + Math.pow( c_y - lidar_point.y, 2 ));
          if(data_distance < 2000){
            if(json_d.objects[j].history.log.length==20 && json_c.data[i].length==3){
              // correspondence_id[c_correspond_id.indexOf(json_c.new_existing_id[i])].push(json_d.objects[j].id);
              var x_closeness = (json_d.objects[j].history.log[json_d.objects[j].history.log.length-1].location.x - json_c.data[i][2][0]
                              + json_d.objects[j].history.log[json_d.objects[j].history.log.length-1-Math.trunc(lidar_flame/2)].location.x - json_c.data[i][1][0]
                              + json_d.objects[j].history.log[json_d.objects[j].history.log.length-1-lidar_flame].location.x - json_c.data[i][0][0])/3;
              var y_closeness = (json_d.objects[j].history.log[json_d.objects[j].history.log.length-1].location.y - json_c.data[i][2][1]
                              + json_d.objects[j].history.log[json_d.objects[j].history.log.length-1-Math.trunc(lidar_flame/2)].location.y - json_c.data[i][1][1]
                              + json_d.objects[j].history.log[json_d.objects[j].history.log.length-1-lidar_flame].location.y - json_c.data[i][0][1])/3;
              var closeness = Math.trunc(Math.sqrt( Math.pow(x_closeness, 2 ) + Math.pow( y_closeness, 2 )));
            }else{
              var x_closeness = json_d.objects[j].history.log[json_d.objects[j].history.log.length-1].location.x - json_c.data[i].slice(-1)[0][0];
              var y_closeness = json_d.objects[j].history.log[json_d.objects[j].history.log.length-1].location.y - json_c.data[i].slice(-1)[0][1];
              var closeness = Math.trunc(Math.sqrt( Math.pow(x_closeness, 2 ) + Math.pow( y_closeness, 2 )));
            }
            if(closeness<min){
              if(min == 9999){
                correspondence_id[c_correspond_id.indexOf(json_c.existing_id[i])].push(json_d.objects[j].id);
                multimodal_data[c_correspond_id.indexOf(json_c.existing_id[i])].x = lidar_point.x;
                multimodal_data[c_correspond_id.indexOf(json_c.existing_id[i])].y = lidar_point.y;
              }else{
                correspondence_id[c_correspond_id.indexOf(json_c.existing_id[i])][1] = json_d.objects[j].id;
                multimodal_data[c_correspond_id.indexOf(json_c.existing_id[i])].x = lidar_point.x;
                multimodal_data[c_correspond_id.indexOf(json_c.existing_id[i])].y = lidar_point.y;
              }
              min = closeness;
          ...
          if(data_distance>4000){
            for(let k=0;k<json_d.objects.length;k++){
              ...
              if(new_data_distance<2000){
                min = new_data_distance;
                correspondence_id[i][1] = json_d.objects[k].id;
                multimodal_data[i].x = json_d.objects[k].cluster.centroid.x;
                multimodal_data[i].y = json_d.objects[k].cluster.centroid.y;
              }
            }
          }
          break;
  ...
  for(let i=0; i < correspondence_id_flag.length; i++){
    ...
    if(correspondence_id_flag[i]==0){
      ...
      if(json_c.existing_id.indexOf(camera_id)!=-1){
        ...
        for(let j=0;j<json_d.objects.length;j++){
          ...
          var data_distance = Math.sqrt( Math.pow(c_x - lidar_point.x, 2 ) + Math.pow( c_y - lidar_point.y, 2 ));
          if(data_distance < 4000){
            if(json_d.objects[j].history.log.length==20 && json_c.data[json_c.existing_id.indexOf(camera_id)].length==3){
              var x_closeness = (json_d.objects[j].history.log[json_d.objects[j].history.log.length-1].location.x - json_c.data[json_c.existing_id.indexOf(camera_id)][2][0]
                              + json_d.objects[j].history.log[json_d.objects[j].history.log.length-1-Math.trunc(lidar_flame/2)].location.x - json_c.data[json_c.existing_id.indexOf(camera_id)][1][0]
                              + json_d.objects[j].history.log[json_d.objects[j].history.log.length-1-lidar_flame].location.x - json_c.data[json_c.existing_id.indexOf(camera_id)][0][0])/3;
              var y_closeness = (json_d.objects[j].history.log[json_d.objects[j].history.log.length-1].location.y - json_c.data[json_c.existing_id.indexOf(camera_id)][2][1]
              ...
              var x_closeness = json_d.objects[j].history.log[json_d.objects[j].history.log.length-1].location.x - json_c.data[json_c.existing_id.indexOf(camera_id)].slice(-1)[0][0];
              var y_closeness = json_d.objects[j].history.log[json_d.objects[j].history.log.length-1].location.y - json_c.data[json_c.existing_id.indexOf(camera_id)].slice(-1)[0][1];
              var closeness = Math.trunc(Math.sqrt( Math.pow(x_closeness, 2 ) + Math.pow( y_closeness, 2 )));
            }
            if(closeness<min){
              correspondence_id[c_correspond_id.indexOf(camera_id)][1] = json_d.objects[j].id;
              multimodal_data[c_correspond_id.indexOf(camera_id)].x = json_d.objects[j].cluster.centroid.x;
              multimodal_data[c_correspond_id.indexOf(camera_id)].y = json_d.objects[j].cluster.centroid.y;
              min = closeness;
            }
          } 
        }
    ...
    for(let j=0;j<correspondence_id.length;j++){
      if(correspondence_id[j][1]==lidar_id){
        if(camera_id!=correspondence_id[j][0]){
          if(c_identification_sub.indexOf(correspondence_id[j][0])==-1 && c_identification_sub.indexOf(camera_id)==-1){
            c_identification_sub.push(camera_id);
            c_identification_sub.push(correspondence_id[j][0]);
            c_identification.push([camera_id,correspondence_id[j][0]]);
          }
        }
      }
  ...
  for(let i=0;i<c_identification.length;i++){
    ...
    if(handle_lidar_id_a!=handle_lidar_id_b && handle_lidar_id_b){
      if(c_correspond_id.indexOf(camera_id_a)!=-1){
        correspondence_id[c_correspond_id.indexOf(camera_id_a)][1] = handle_lidar_id_a;
        // multimodal_data[c_correspond_id.indexOf(camera_id_a)][1] = handle_lidar_id_a;
      }
      if(c_correspond_id.indexOf(camera_id_b)!=-1){
        correspondence_id[c_correspond_id.indexOf(camera_id_b)][1] = handle_lidar_id_b;
        // multimodal_data[c_correspond_id.indexOf(camera_id_b)][1] = handle_lidar_id_b;
      }
  ...
  // console.log(c_identification);
  console.log(multimodal_data);
  console.log(correspondence_id);
  // console.log(json_c.existing_id);
  // one_display_sensor_data(complementary_data);
}

function integration_3(json_c,json_d){
  complementary_data =[];
  if(json_c.time_history.length==3){
    // console.log(json_c.time_history[0]);
    var camera_flame_time = to_date_c(json_c.time_history[2]).getTime() - to_date_c(json_c.time_history[0]).getTime();
    ...
    for(let i=0;i<json_c.existing_id.length;i++){
      if(c_correspond_id.indexOf(json_c.existing_id[i])==-1){
        // console.log(json_c.new_existing_id);
        correspondence_id.push([json_c.existing_id[i]]);
        multimodal_data.push({"id" : json_c.existing_id[i], "time" : json_c.det_time, "status" :  "camera" ,"x" : json_c.data[i].slice(-1)[0][0], "y" : json_c.data[i].slice(-1)[0][1], "history" : []});
        c_correspond_id.push(json_c.existing_id[i]);
        var c_x = json_c.data[i].slice(-1)[0][0];
        // console.log(c_x);
        // var c_move_x = c_x - json_c.data[i][0][0];
  ...
  for(let i = 0; i<correspondence_id.length; i++){
    ...
    for(let j=0; j<json_d.objects.length; j++){
      if(lidar_id==json_d.objects[j].id){
        multimodal_data[i].status = "lidar";
        multimodal_data[i].x=json_d.objects[j].cluster.centroid.x;
        multimodal_data[i].y=json_d.objects[j].cluster.centroid.y;
      }
    }
    for(let j = 0; j<correspondence_id.length; j++){
      if(i!=j && lidar_id == correspondence_id[j][1]){
        if(c_correspond_id.indexOf(camera_id)!=-1){
          multimodal_data[i].status = "camera";
          multimodal_data[i].x=json_c.data[c_correspond_id.indexOf(camera_id)].slice(-1)[0][0];
          multimodal_data[i].y=json_c.data[c_correspond_id.indexOf(camera_id)].slice(-1)[0][1];
          console.log("re")
        }
      }
    }
  }
  for(let i=0; i<multimodal_data.length; i++){
    var c_id = multimodal_data[i].id;
    if(json_c.existing_id.indexOf(c_id)==-1){
      multimodal_data.splice(i,1);
      i--;
    }
  ...
  for(let i=0; i<multimodal_data.length; i++){
    multimodal_data[i].history.push([multimodal_data[i].x,multimodal_data[i].y]);
    if(multimodal_data[i].history.length>10){
      multimodal_data[i].history.shift();
    }
  }
  // console.log(c_identification);
  console.log(multimodal_data);
  console.log(correspondence_id);
  display_multimodal_data(multimodal_data);
  // console.log(json_c.existing_id);
  // one_display_sensor_data(complementary_data);
}
