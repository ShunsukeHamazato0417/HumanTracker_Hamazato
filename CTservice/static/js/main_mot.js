...
########################*/
// Worker定義 
const worker = new Worker(new URL('./worker_mot.js', import.meta.url));
worker.postMessage({
	ty: ty
})
...
for (var i=1; i<=sensor_n; i++){
  ...
  sensor_color_box[i] = new PIXI.Graphics()
			...
			.drawRect(0, 0, 20, 20)
			.endFill();
  sensor_name[i] = new PIXI.Text('camera-'+i,
			{fontFamily : 'Arial',
			 fontSize : 15,
			 fill : 0x000000,
			 algin : 'center'});
...
function display_sensor_data(json){
  ...
  console.log(json);
  displaying = true;
  time_display.text = "Time: "+ json[0].det_time.year +"/" + json[0].det_time.mon + "/"+json[0].det_time.day+"  "+json[0].det_time.hour+":"+json[0].det_time.min+":"+json[0].det_time.sec+"."+json[0].det_time.misec;
...
function FpsDisplay(){
  ...
  for (var i=1; i<lags.length; i++){   
    sensor_lag[i].text = 'Lag: ' + Math.floor(lags[i]/sensor_n)/1000 + 's';
    lags[i] = 0;
  }
