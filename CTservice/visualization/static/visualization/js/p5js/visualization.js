...
/*sock.onmessage = function(m){
  ...
  for (var i=1; i<human.length; i++){
    ...
    humans.push(new HumanObject(human[i].centroid[0], human[i].centroid[1], 
                                human[i].size, human[i].data, i, hue, DDflg));
  }
}*/
  ...
  draw(p, frame){
    if (this.state != "disappearance" && this.state != "hidden"){
      ...
      if (this.type == "human"){
        ...
        p.line(0, 0,Math.sqrt(this.speed * 10000), 0);
        p.ellipse(0,
                  0,
                  this.size*this.data_reduce_rate/4, this.size*this.data_reduce_rate/4);
        p.pop();
        p.stroke(255);
        p.fill(0);
