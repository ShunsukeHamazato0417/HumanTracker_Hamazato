...
var file_text = [["time", "real_datas", "datas", "lag", "objects", "fps"]];
var real_datas = 0;
var pre_data_time;
var pre_get_time = new Date();
var fps = 1;
...
sock.onmessage = function(m){
  ...
  if (time != pre_data_time){
    ...
    lag = lag / sensor_n;
    var wer = now.getTime()-pre_get_time.getTime();
    if (wer > 40){
      fps = 1000 / wer;
    }
    console.log(fps);

    var file_tmp = [time, human[0].real_datas, human[0].datas, lag, human.length-1, fps]
...
　/* 初期処理 */
  ...
  p.draw = function(){
    ...
    sock.send("send_HumanDetectedData3");
...
var info = function(p){
  ...
  p.setup = function(){
    ...
    display_point  = p.createCheckbox('点群表示', true);
  ...
  draw(p, frame){
    if (this.state != "disappearance" && this.state != "hidden"){
      ...
      if (this.type == "human"){
        ...
        p.ellipse(0,
                  0,
                  600*this.data_reduce_rate/4, 600*this.data_reduce_rate/4);
        p.pop();
        p.stroke(255);
        p.fill(0);
        if (DIflg){
...
function file_export(){
  exportCSV(file_text);
  file_text = [["real_datas", "datas", "lag", "objects", "fps"]];
}
