...
var messages = 0;
var start;
var timelags = [0, 0, 0, 0, 0, 0, 0, 0, 0]
var sock = new WebSocket('ws://192.168.50.12:8081');
var file_text = [["time", "real_datas", "datas", "lag", "objects"]];
var real_datas = 0;
var pre_data_time;
...
sock.onmessage = function(m){
  ...
  var human = JSON.parse(m.data);
  time = human[0].time;
  if (time != pre_data_time){
    if (humans.length>1){
      var len = humans.length - 1;
      humans.splice(1, len);
    }

    real_datas = human[0].real_datas;
    for (var i=1; i<human.length; i++){
      hue = 360 * i / (human.length - 1);
      humans.push(new HumanObject(human[i].id, human[i].type, human[i].centroid, human[i].size, 
                                  human[i].data, human[i].speed, human[i].direction, human[i].state, frames));
    } 
    ...
    for (var i=0; i<sensor_n; i++){
      var get = to_date(human[0].get_times[i])  
      var lag = lag + now.getTime() - get.getTime();
      //timelags[i] = timelags[i] + lag;
      //console.log((i+1) + ": " + timelags[i]/messages);
    }
    lag = lag / sensor_n;
    console.log(lag);

    var file_tmp = [time, human[0].real_datas, human[0].datas, lag, human.length-1]
    file_text.push(file_tmp);
    pre_data_time = time;
  }
}
...
var canvas = function(p) {
  ...
  p.preload = function(){
    ...
    bg_img = p.loadImage('../../../media/visualization/floormap.png');
    //bg_W = 1439;	// 背景画像の横幅
    bg_W = 1235;
    //bg_H = 525;		// 背景画像の縦幅
    bg_H = 566
...
　/* 初期処理 */
  ...
  p.draw = function(){
    ...
    if (display_bg_img_Flg){
      p.tint(255, 127);
      p.image(bg_img, 
              (1400-display_x)*expantion_rate/max_expantion_rate, 
              ( 180-display_y)*expantion_rate/max_expantion_rate,
    ...
    /*for (var i=1; i<humans.length; i++){
      var hue = 360 * i / (humans.length - 1);
      p.text(p.str(i), humans[i][0][0]/80, humans[i][0][1]/80);
      for (var j=0; j<humans[i].length; j++){
       p.fill(hue, 200, 200);
       p.ellipse(humans[i][j][0]/80, humans[i][j][1]/80, 3, 3);
      }
    }*/

    for (var i=1; i<humans.length; i++){
      humans[i].draw(p, frames);
    }

    sock.send("send_HumanDetectedDataTo");

    frames = frames + 1;
    var end = performance.now();
    var fps = Math.round(frames / (end - start) * 1000000) / 1000;
    p.fill(0);
    p.noStroke();
    p.text("fps: " + p.str(fps), windowW - 200, windowH - 10);

    p.text("real_datas: " + p.str(real_datas), windowW -200, 30);
  }
...
var info = function(p){
  ...
  p.setup = function(){
    ...
    display_point  = p.createCheckbox('追加背景表示', true);
    display_id     = p.createCheckbox('ID表示', true);
  ...
  p.draw = function(){
    ...
    data_save.position(windowW+10, 80);
    data_save.mousePressed(file_export);

    // センサ〜データ間の線の表示/非表示のチェックボックス
    display_Data[0].position(windowW+15, 100);
    display_Data[0].changed(p.displayDataLine);    

    // 背景画像の表示/非表示のチェックボックス
    display_bg_img.position(windowW+15, 120);
    display_bg_img.changed(p.displayBgImage);

    display_sensor.position(windowW+15, 140);
    display_sensor.changed(p.displaySensor);

    display_point.position(windowW+15, 160);
    display_point.changed(p.displayPoint);

    display_id.position(windowW+15, 180);
    display_id.changed(p.displayId);
...
class HumanObject{
  ...
  constructor(id, type, centroid, size, data, speed, direction, state, appear){
    this.id = id;
    this.type = type;
    ...
    this.state = state;
    this.DDflg = DDflg;
    this.data_reduce_rate = 1/20;
    this.appear = appear;
  }

  draw(p, frame){
    if (this.state != "disappearance" && this.state != "hidden"){

      if (this.type == "human"){
        p.push();
        p.translate((this.centroid[0]*this.data_reduce_rate-display_x)*expantion_rate/max_expantion_rate,
                    (this.centroid[1]*this.data_reduce_rate-display_x)*expantion_rate/max_expantion_rate);
        p.rotate(this.direction * p.PI / 180)

        //var x = (frame-this.appear) * this.speed * 1000

        p.noStroke();
        var hue = (21 * this.id) % 360;
        p.fill(hue, 200, 200);    
        //p.fill(255);
        p.stroke(hue, 200, 200);
        p.line(0, 0,Math.sqrt(this.speed * 30000), 0);
        p.ellipse(0,
                  0,
                  1200*this.data_reduce_rate/4, 1200*this.data_reduce_rate/4);
        p.pop();
        p.stroke(255);
        p.fill(0);
        if (DIflg){
          p.text(p.str(this.id), (this.centroid[0]*this.data_reduce_rate-display_x)*expantion_rate/max_expantion_rate,
                                 (this.centroid[1]*this.data_reduce_rate-display_x)*expantion_rate/max_expantion_rate);
        }
        ...
        if (this.DDflg){
          ...
          for (var i=0; i<this.data.length; i++){
            p.ellipse((this.data[i][0]*this.data_reduce_rate-display_x)*expantion_rate/max_expantion_rate,
                      (this.data[i][1]*this.data_reduce_rate-display_x)*expantion_rate/max_expantion_rate,
                      3,3);
          }
        }
      }
...
class SensorObject {
  ...
  constructor(x, y, d, data, sensor_id, hue, DLflg, DSflg){
    ...
    this.sensor_x = x;			// センサのx座標
    this.sensor_y = y;			// センサのy座標
    ...
    this.data_reduce_rate = 1/20;	// 表示の倍率
  }
  ...
  draw(p){
    ...
    if (this.displayFlg){
      for (var i=0; i<this.data.length; i++){
        ...
        p.line((this.sensor_x*this.data_reduce_rate-display_x)*expantion_rate/max_expantion_rate,
	       (this.sensor_y*this.data_reduce_rate-display_y)*expantion_rate/max_expantion_rate, 
	       (this.data[i][0]*this.data_reduce_rate-display_x)*expantion_rate/max_expantion_rate,
	       (this.data[i][1]*this.data_reduce_rate-display_y)*expantion_rate/max_expantion_rate);
        }   
    ...
    // 原点をセンサの位置に変更
    p.push();
    p.translate((this.sensor_x*this.data_reduce_rate-display_x)*expantion_rate/max_expantion_rate,
                (this.sensor_y*this.data_reduce_rate-display_y)*expantion_rate/max_expantion_rate);
...
function file_export(){
  exportCSV(file_text);
  file_text = [["real_datas", "datas", "lag", "objects"]];
}
