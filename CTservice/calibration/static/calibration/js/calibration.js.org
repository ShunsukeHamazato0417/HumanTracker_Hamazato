...
/*sock.onmessage = function(m){
  ...
  for (var i=1; i<=sensor_n; i++){
    hue = 360 * i / sensor_n; 
      ...
      var tmp1 = [];
      var tmp2 = [];
      tmp1.push(createButton('+10'));
      tmp1.push(createButton('+1'));
      tmp2.push(tmp1);
      var tmp1 = [];
      tmp1.push(createButton('+10'));
      tmp1.push(createButton('+1'));
    ...
    objlist.push(new SensorObject(sensor[i].x, sensor[i].y, sensor[i].direction, 
                                  sensor[i].data, i, hue, increase[i], decrease[i]));
...
var info = function(p){
  ...
  p.draw = function(){
    ...
    data_update.mousePressed(dataUpdate);
...
class SensorObject {
  ...
  /* 各パラメータ設定 */
  constructor(x, y, d, data, sensor_id, hue, increase, decrease){
    // センサのパラメータ
